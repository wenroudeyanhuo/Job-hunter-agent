...
func BuildLocalAgentChatReply(input string, context AgentChatContext) AgentChatReply {
	...
	if containsAnyText(text, "模型", "deepseek", "model", "llm") {
		if context.ModelEnabled {
			reply.Content = "我已经检测到模型配置，可以用模型理解更自由的对话；本地规则仍会作为兜底。"
		} else {
			reply.Content = "现在我处于本地规则模式，还没有检测到模型密钥。配置 DeepSeek 或 OpenAI-compatible 模型后，我就能切换到模型对话。"
		}
		return reply
	}
	if containsAnyText(text, "采集", "抓取", "刷新岗位", "最新岗位", "crawl", "run crawl") {
		reply.Content = "可以，我可以帮你发起采集；我会先把采集作为待批准动作展示，你批准后我再执行，并刷新今日任务。"
